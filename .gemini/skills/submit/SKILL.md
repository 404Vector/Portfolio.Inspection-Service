...
### Step 1 — Branch 확인
만약 현재 브랜치가 `main` 또는 `master`인 경우, 새로운 브랜치를 생성해야 합니다. `branch` 스킬을 호출하여 자동으로 처리합니다.

### Step 2 — 미커밋 변경사항 처리
미커밋된 변경사항이 있다면 커밋해야 합니다. `commit` 스킬을 호출하여 자동으로 처리합니다.

### Step 3 — PR 생성
1. `git log main..HEAD` 와 변경된 파일 목록을 기반으로 PR의 제목과 본문 초안을 작성합니다.
2. `gh pr create` 명령을 실행하여 PR을 생성합니다.
3. 생성된 PR의 URL과 번호를 출력합니다.
