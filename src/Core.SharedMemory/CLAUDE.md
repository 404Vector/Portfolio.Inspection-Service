...
- `SharedMemoryRingBufferReader` — Reader(Consumer) 구현.
- `FrameInfo` — 프레임 메타데이터 레코드 (gRPC 변환의 원본 데이터)
- `RingBufferOptions` — 링버퍼 설정 (슬롯 수, 해상도, PixelFormat 등)
- `RingBufferLayout` / `RingBufferHeader` / `SlotHeader` / `SlotState` — 메모리 레이아웃 상수 및 구조체
...
- unsafe 블록은 레이아웃 접근(포인터 연산, `Unsafe.As` 등)에만 사용하고, 비즈니스 로직은 safe 코드로 분리한다.
- `SharedMemoryRingBuffer`(Writer)와 `SharedMemoryRingBufferReader`(Reader)는 서로 직접 참조하지 않는다 — 공유 상태는 MMF를 통해서만 교환한다.
- `SlotState` 전이는 명시적 상태 머신 규칙을 따른다 (예: `Empty → Writing → Ready → Reading → Empty`).
- `RingBufferHeader`, `SlotHeader`는 `readonly struct`로 정의하여 불의의 수정을 방지한다.
