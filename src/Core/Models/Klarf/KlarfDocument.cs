...
///
/// KlarfDocument는 DB 영속 모델이 아닌 출력 포맷 모델입니다.
/// KLARF에는 WaferID/LotID 같은 식별자 외에 SampleSize(웨이퍼 직경),
/// DiePitch, DieOrigin, NotchFlatLocation 등 웨이퍼 물리 정보가 필요하므로
/// WaferInfo 전체를 보유합니다.
/// </summary>
...
/// <param name="Wafer">
/// 검사 대상 웨이퍼 정보.
/// KLARF LotID, WaferID, SampleSize(직경), SampleOrientationMarkType,
/// NotchFlatLocation, DiePitch, DieOrigin, SampleCenterLocation 생성에 사용.
/// </param>
...
public record KlarfDocument(
  ...
  /// <param name="wafer">검사 대상 웨이퍼 정보. Result에는 WaferId만 있으므로 호출자가 조회하여 전달한다.</param>
